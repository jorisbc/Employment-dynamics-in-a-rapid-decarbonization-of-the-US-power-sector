...
#### GET ADVANCED COST DATA ##########
SCENARIO.IDX <- 'Advanced'
######################################
...
battery.l <- d[d$technology == 'Utility-Scale Battery Storage' & d$techdetail == "4Hr Battery Storage" & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
battery <- dcast(dat = battery.l, formula = years ~ core_metric_parameter, value.var = 'value')

bio.l <- d[d$technology == 'Biopower' & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
bio <- dcast(dat = bio.l, formula = years ~ core_metric_parameter, value.var = 'value')

coal.l <- d[d$technology == 'Coal_FE' & d$techdetail == "newAvgCF" & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
coal <- dcast(dat = coal.l, formula = years ~ core_metric_parameter, value.var = 'value')

csp.l <- d[d$technology == 'CSP' & d$techdetail == "Class3" & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
csp <- dcast(dat = csp.l, formula = years ~ core_metric_parameter, value.var = 'value')

pvcomm.l <- d[d$technology == 'CommPV' & d$techdetail == "Class5" & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
pvcomm <- dcast(dat = pvcomm.l, formula = years ~ core_metric_parameter, value.var = 'value')

pvres.l <- d[d$technology == 'ResPV' & d$techdetail == "Class5" & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
...
pvutil.l <- d[d$technology == 'UtilityPV' & d$techdetail == "Class5" & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
pvutil <- dcast(dat = pvutil.l, formula = years ~ core_metric_parameter, value.var = 'value')

gasct.l <- d[d$technology == 'NaturalGas_FE' &  d$techdetail == 'CTAvgCF' & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
gasct <- dcast(dat = gasct.l, formula = years ~ core_metric_parameter, value.var = 'value')

gascc.l <- d[d$technology == 'NaturalGas_FE' &  d$techdetail == 'CCAvgCF' & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
gascc <- dcast(dat = gascc.l, formula = years ~ core_metric_parameter, value.var = 'value')

gasccccs.l <- d[d$technology == 'NaturalGas_FE' &  d$techdetail == 'CCCCSAvgCF' & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
gasccccs <- dcast(dat = gasccccs.l, formula = years ~ core_metric_parameter, value.var = 'value')

hydro.l <- d[d$technology == 'Hydropower' &  d$techdetail == 'NPD1' & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
hydro <- dcast(dat = hydro.l, formula = years ~ core_metric_parameter, value.var = 'value')

phs.l <- d[d$technology == "Pumped Storage Hydropower" &  d$techdetail == 'Class 3' & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
phs <- dcast(dat = phs.l, formula = years ~ core_metric_parameter, value.var = 'value')

windon.l <- d[d$technology == "LandbasedWind" & d$techdetail == 'Class4' & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
windon <- dcast(dat = windon.l, formula = years ~ core_metric_parameter, value.var = 'value')

windoff.l <- d[d$technology == "OffShoreWind" & d$techdetail == 'Class3' & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
windoff <- dcast(dat = windoff.l, formula = years ~ core_metric_parameter, value.var = 'value')

geo.l <- d[d$technology == "Geothermal" & d$techdetail == 'HydroFlash' & d$scenario == SCENARIO.IDX & d$crpyears == 30,]
geo <- dcast(dat = geo.l, formula = years ~ core_metric_parameter, value.var = 'value')

## NUCLEAR IS ONLY EXCEPTION WHERE THERE IS ONLY MODERATE
...
techcost.ADVANCED <- rbind.data.frame(
...
colnames(techcost.ADVANCED)[4:5] <- c('FOM','VOM')



############################################################################################################################################

#### GET CONSERVATIVE COST DATA ##########
SCENARIO.IDX <- 'Conservative'
######################################
...
techcost.CONSERVE <- rbind.data.frame(
  cbind.data.frame(technology = 'Coal', coal),
  cbind.data.frame(technology = 'Gas CC', gascc),
  cbind.data.frame(technology = 'Gas CC CCS', gasccccs),
  cbind.data.frame(technology = 'Gas CT', gasct),
...
colnames(techcost.CONSERVE)[4:5] <- c('FOM','VOM')



write.csv(techcost.ADVANCED, 'data_out/cost_nrel_advanced.csv')
write.csv(techcost.CONSERVE, 'data_out/cost_nrel_conservative.csv')